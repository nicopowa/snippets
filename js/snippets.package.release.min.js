 ...
 * SNIPPETS PACKAGE RELEASE v0.3.0
 * 01/12/2024 02:00:33
 */
(function(){
...
F(this.A,"cmds","high");var a=F(this.C);F(a,"cmd","pin");a=F(this.C);this.Na=F(a,"cmd","ver");a=F(this.C);this.Ja=F(a,"cmd","siz");F(this.F,"cmd","new");F(this.F,"cmd","abt");this.ya=F(this.F,"cmd","dmp");F(this.F,"cmd","imp");this.Z=new Map;this.D.forEach(b=>{let c=F(this.U,"lwrap","wrap-"+b);F(c,"lcode");F(c,"ltool");this.Z.set(b,c)});this.K=F(this.U,"cmds","down");this.P=F(this.K,"del");F(this.P,"cmd","del-ask");F(this.P,"cmd","del-no");F(this.P,"cmd","del-yes");F(this.K,"spc");ja(this);ka(this);
"serviceWorker"in navigator&&navigator.serviceWorker.register("js/snippets.package-service.release.min.js",{});K(this)}Ca(){let a=this.m.innerHTML;ua(this,a).then(b=>{b&&b!==a&&(this.m.innerHTML=b,S(this))})}Da(){R(this.da,"none")}O(){D(this.da,"none")}ca(){let a=this.L.value;va(this.N).forEach(b=>{var c=a&&!b.textContent.includes(a);b.classList.toggle("hide",c)})}ba(){this.O();L(this);O();M(this)}pa(){window.open("https://github.com/nicopowa/snippets","_blank")}async ea(){if(this.h){var a=this.l.find(b=>
b.i===this.h);a&&(this.s++,a.t=this.m.innerHTML,a.v=this.s,E("[href='#"+this.h+"']",this.N).innerHTML=a.t)}else this.h=Date.now().toString(36)+Math.random().toString(36).slice(2),this.s=0,a={i:this.h,v:this.s,t:this.m.innerHTML},this.l.push(a),H(this,a),Q(this),ya(this.h);window.localStorage.setItem("snippet-"+this.h,await X(this,{i:this.h,v:this.s}));Y(this);T(this);this.R();Z(this.V)}ua(a){let b=a.getAttribute("snip");a=a.getAttribute("lang");let c=this.g[a].findIndex(d=>d.id==b);a=this.u[a][c];
a.classList.contains("clps")&&R(a,"clps")}La(a){let b=a.getAttribute("snip");a=a.getAttribute("lang");let c=this.g[a].findIndex(d=>d.id==b);a=this.u[a][c];a.classList.toggle("clps",void 0);R(E(".rem",a),"ask")}qa(a){a=a.getAttribute("lang");P(this,a,{name:a+"-"+this.o[a].length,type:""},"",this.Z.get(a).firstChild);S(this)}sa(a){D(a.parentElement,"ask")}Fa(a){a=a.parentNode;let b=a.getAttribute("snip");a=a.getAttribute("lang");let c=this.g[a].findIndex(d=>d.id==b);ta(this,a,c);S(this)}Ha(a){R(a.parentElement,
"ask")}ra(a){D(a.parentElement,"ask")}xa(){wa(this,'delete "'+this.m.textContent+'" ?').then(a=>{this.R();a&&(this.l.splice(this.l.findIndex(b=>b.i===this.h),1),Y(this),E("[href='#"+this.h+"']",this.N).parentNode.remove(),window.localStorage.removeItem("snippet-"+this.h),this.ba())})}R(){R(this.P,"ask")}Ea(){this.A.classList.contains("stick")?R(this.A,"stick"):D(this.A,"stick")}Ma(){}async wa(){let a=location.origin+location.pathname+"#"+await X(this);navigator.canShare?navigator.share({title:this.m.textContent,
url:a}):navigator.clipboard&&navigator.clipboard.writeText(a).then(()=>Z(this.va))}za(){let a=this.l.reduce((c,d)=>{d=d.i;c[d]=window.localStorage.getItem("snippet-"+d)||"";return c},{[this.I]:this.l}),b=document.createElement("a");U(b,{href:"data:application/json;charset=utf-8,"+encodeURIComponent(JSON.stringify(a)),download:this.I+".json"});b.style.display="none";document.body.appendChild(b);b.click();b.remove();Z(this.ya)}Aa(){let a=document.createElement("input");a.type="file";a.accept="application/json";
a.style.display="none";document.body.appendChild(a);a.addEventListener("change",()=>{if(a.files.length){let b=a.files[0],c=new FileReader;c.addEventListener("loadend",d=>{try{let e=JSON.parse(d.target.result);e.snippets.filter(f=>e[f.i]).forEach(f=>{let h=this.l.find(k=>k.i===f.i);if(h&&h.v>h.v||!h)window.localStorage.setItem("snippet-"+f.i,e[f.i]),H(this,f),this.l.push(f),Y(this)})}catch(e){}});c.readAsText(b)}a.remove()});a.click()}};window.addEventListener("load",()=>new za);}).call(this);
